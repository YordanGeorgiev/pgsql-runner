...
doRunPgsqlScripts(){
   ...
   # run the sql save the result into a tmp log file
	psql -X -U "$pgsql_user" -v pgsql_db="$pgsql_db" -f "$sql_script" postgres > "$tmp_log_file" 2>&1
   ...
   while read -r sql_script ; do (
		...
		echo -e '\n\n'
		# run the sql save the result into a tmp log file
		psql -X -U "$pgsql_user" -v pgsql_db="$pgsql_db" -f "$sql_script" "$pgsql_db"> "$tmp_log_file" 2>&1
