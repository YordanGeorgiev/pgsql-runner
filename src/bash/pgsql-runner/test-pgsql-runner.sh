#!/bin/bash 
# file: src/bash/pgsql-runner/test-pgsql-runner.sh
      ...
      doLogTestRunEntry 'START'
		while read -r test_file ; do (
            ...
            then
               ...
               $function_name
               doLog "INFO test-pgsql-runner loop exit_code: $exit_code" 
               doLogTestRunEntry 'INFO' ' '$(date "+%H:%M:%S")
		...
		done < <(find src/bash/pgsql-runner/tests -type f -name '*.sh')

	);
	done < <(cat $product_instance_dir/src/bash/pgsql-runner/tests/run-pgsql-runner-tests.lst)
...
doSetVars(){
   ...
   . "$product_instance_dir/src/bash/pgsql-runner/funcs/log-test-run-entry.func.sh"
...
#----------------------------------------------------------
# Purpose:
# a simplistic tester for the simplistic bash stub pgsql-runnerer
#----------------------------------------------------------
#
#----------------------------------------------------------
...
#----------------------------------------------------------
#
# eof file: src/bash/pgsql-runner/test-pgsql-runner.sh
